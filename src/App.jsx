...
// timeoutMs bajo (10s) a proposito: el dialogo nativo de huella de Android a
// veces se queda colgado sin resolver la promesa de WebAuthn (fallo conocido
// de Play Services / Credential Manager). Antes, sin este limite, la pantalla
// se quedaba en "comprobando" para siempre porque tryingBio nunca volvia a
// false. Con Promise.race, la app abandona sola y deja pasar al PIN aunque el
// dialogo del sistema se haya colgado por su cuenta.
async function verifyBiometric(credId, timeoutMs = 10000) {
  try {
    ...
    const getPromise = navigator.credentials.get({
      publicKey: {
        ...
        userVerification: "required", timeout: timeoutMs,
      },
      signal: controller.signal,
    });
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => { try { controller.abort(); } catch {} reject(new Error("biometric-timeout")); }, timeoutMs);
    });
    const assertion = await Promise.race([getPromise, timeoutPromise]);
    return !!assertion;
  } catch { return false; }
}
// Formato legado del PIN (base64 sin sal, reversible). Se mantiene solo para
// poder seguir verificando PIN guardados con este esquema antes del cambio a
// hash real; ya no se usa para guardar PIN nuevos.
function obfuscatePin(pin) { return btoa(`cc-${pin}-lock`); }
// Hash real del PIN (SHA-256 vía Web Crypto API) con sal aleatoria por
// instalacion. A diferencia de obfuscatePin(), esto no es reversible.
...
// Verifica el PIN soportando los dos formatos que puede haber en el storage
// de un dispositivo: el nuevo (hash+sal, cuando appLock.pinSalt existe) y el
// antiguo (base64, de antes de la migracion). Este es el arreglo clave del
// bug de "PIN correcto marcado como invalido": antes, si el PIN se habia
// guardado con un esquema y el codigo activo comparaba con el otro, nunca
// podian coincidir sin importar que el PIN escrito fuera correcto.
async function verifyPin(pin, appLock) {
  if (appLock.pinSalt) return (await hashPin(pin, appLock.pinSalt)) === appLock.pin;
  return obfuscatePin(pin) === appLock.pin;
}
...
function LockScreen({ appLock, onUnlock }) {
  ...
  const press = async (d) => {
    ...
    setEntry(next);
    if (next.length >= 4) {
      const ok = await verifyPin(next, appLock);
      if (ok) { onUnlock(); }
